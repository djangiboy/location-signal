# Solution Frame v5 — Location Signal (Two Confidences + Integrity Channel)

**Drafted:** 2026-04-22
**Predecessor:** `solution_frame_v4.md` (kept; v5 preserves the two-confidence mental model, adds cross-OS integrity routing, fixes decay gaps, rewrites payment mechanics)
...
The data also surfaces a third corruption channel (partner-side booking-coord gaming) that the integrity channel in §6a now closes.
...
- *Wiom:* gets a learning loop that tightens capture, serviceability, and partner integrity with every install. The 25.7% drift, the 1.92 calls per pair, the 77.5% within-ANC confusion, the 31pp install-gap at the same GNN probability on gaming partners are not permanent taxes — they are what the system pays today because nothing flows back.
...
Three parties are present at every step: **Wiom, partner (CSP), customer.** Each holds a different fragment of ground truth. The customer knows where his home is. The partner knows where he can actually install. Wiom sits between them and today lets neither speak before the promise is made — and also lets no upstream OS warn the matchmaker when a partner is gaming the system.
...
Promise Maker is the commitment engine inside Genie (Wiom's matchmaking brain). Think of Genie as the component that decides whether to make a promise, and to which partner to route it. The technical internals (how Genie separates belief from governance from external events) are in the appendix — the body of this document only needs the idea that **Genie already has a structured channel for consuming events from other Wiom systems**, and that signals flow in through that channel without re-implementing them inside Genie.

### 2.6 — The wider Wiom OS universe

Genie is one of fourteen systems (OSes) at Wiom. This frame touches four of them:

- **Demand & Allocation (DA-OS)** — where Genie's decision lands; the allocation engine that routes accepted bookings to partners.
- **Quality OS (Q-OS)** — owns partner quality state (install integrity, rework, complaint pattern).
- **Exit Enforcement OS (XS-OS)** — owns partner lifecycle and terminal-integrity state.
- **Enforcement OS** — owns event-time integrity breaches that need to fire fast, before slow quality patterns crystallise.

These OSes already have an integrity-event contract for talking to Genie. §6a below uses that existing contract to carry partner-integrity signal. **No new interface is invented — this is a new payload on an existing channel.**

### 2.7 — Phased rollout — simpler logic first, models later

The frame describes the endpoint: two belief-model gates plus an integrity channel. But the belief models (BM1 KDE and BM2 GNN) are R&D assets that need production-wiring, calibration, and operational sign-off before they can be consulted at the gate. **The build ships in phases so business value lands before model activation completes.**
...
- **Phase 4 (Post integrity contract finalisation):** §6a cross-OS integrity shocks fire at the gate.

Each phase's capabilities are tagged by phase in §12. **The user-facing mental model does not change across phases — only the sophistication of the tier computation inside Gate 2 grows.**
...
### Claim 4 — Partner-side booking-coord gaming is a third root cause (new in v5)

**Evidence.** Within a single GNN probability decile (same modelled install likelihood), partners with >90% of their historical bookings' coords coincident with splitters install at **5.0%**, while partners with <10% splitter-share install at **35.8%** — a **30.8pp gap at the same GNN probability**. *(Master story Appendix lines 371-383.)*

**What this means.** This gap is not customer-side capture noise. It is partners submitting booking coords that coincide with splitter infrastructure to pass the 25m gate, regardless of whether the customer actually lives there. It is invisible to Gate 1 (customer is genuinely at home in his own view) and invisible to Gate 2 (polygon contains the splitter by construction). **The location signal has three corruption sources, not two: customer-side capture, system-side ranking, and partner-side gaming.** The first two are solved inside Genie. The third is solved by routing integrity signals from Q-OS, XS-OS, and Enforcement OS into Genie's S stock — §6a.

### Together, these four claims shape the build

Claims 1 + 2 justify **activating the R&D belief models as-is** — they are robust to customer-side noise and they price the polygon effect the data surfaces. Claim 3 justifies **rebuilding customer-side capture**. Claim 4 justifies **extending the Shock Ledger Intake contract** to carry partner-integrity shocks into Genie. Three workstreams, all parallel.

---

## §4 — The mental model: two confidences + one integrity shock channel

This is the reframe. v3 laid out 31 capabilities in four groups. v4 collapsed them onto two belief models evaluated sequentially. v5 preserves that, and adds a cross-OS integrity channel that runs *alongside* both gates.
...
### 4.3 — Why integrity shocks are orthogonal, not another gate

Integrity — is this partner gaming the allocation? — is **not a per-booking belief.** It is a **per-partner state** that can fire at any time, governed by three external Wiom OSes, and consumed by Genie at the gate as a **shock-driven veto**, not as a continuous score. Integrity never becomes "a third gate with HIGH/MID/LOW tiers" because that would put partner-state logic inside Genie. It belongs in Q-OS / XS-OS / Enforcement OS; Genie just reads the shock ledger (S) and acts on it.
...
```
         ...
         promise time      refund if
           ...
           ┌────────────────────────────────────┐
           │  INTEGRITY SHOCK CHANNEL (§6a)     │
           │  Q-OS + XS-OS + Enforcement → S    │
           │  Fires at gate via integrity_block │
           │  Orthogonal to Gate 1 / Gate 2     │
           └────────────────────────────────────┘
```

### 4.5 — The three confidence surfaces, summarised

| Surface | Question | Owned by | Lives in | Crosses into Genie as |
|---|---|---|---|---|
| User Address Confidence (new) | Is this really home? | Genie B (per-lead) | Genie | N/A — internal belief |
| Serviceability Confidence (R&D, now activated) | Can we serve here? | Genie B (per-lead × partner) | Genie | N/A — internal belief |
| Partner Integrity (new via existing contract) | Is this partner honest? | Q-OS + XS-OS + Enforcement OS (per-partner) | Outside Genie | S (shock ledger), via IC-INT-10 |

The first two are Genie's own belief stocks. The third is a consumption of external state, via an already-specified contract.
...
**Every scoring artefact is stored as a triplet, not a scalar:** `(tier, evidence_count, last_observed)` per signal class, and per booking. A customer who picked 2 landmarks with 0 probes encountered is not the same booking as one who picked 2 landmarks and survived 3 probe rounds. *(Addresses v4 review: quality ≠ confidence — Donna's durable principle #3.)*
...
The "CRE or AI verification call" referenced above is not a hand-wave. It is a **Support Resolution OS (SR-OS)** handoff with a named queue, an SLA (10 minutes for first touch; 24h for resolution), and a volume ceiling (engineered to <5% of MID cohort — above which the queue must surface a Shock to CP for ops attention). This closes the hanging node Geoff flagged on v4. Engineering mechanics ship in the spec.
...
Wiom has spent the past year building a rich R&D stack. This build activates it in phases (see §2.7).
...
- **LOW** — reject, booking routed to the expansion-demand queue. **Exploration quota (B7) lets a bounded fraction through with explicit partner rotation** — rotation prevents "route-to-nearest" from reinforcing the same partner's blind spot (v4 Donna failure mode). See §13 for rotation mechanics.
...
## §6a — Partner Integrity Channel (phased: activates in Phase 4)

### 6a.1 — The problem v4 did not address

The master story Appendix reveals: within one allocation-model probability decile, >90% splitter-share partners install at **5.0%** vs <10% splitter-share partners at **35.8%** — a **30.8pp gap at the same modelled probability**. This is partners gaming the allocation by submitting bookings whose coords coincide with splitters. Gate 1 cannot see it (customer may be genuinely home). Gate 2 cannot see it (polygon contains the splitter). **The signal lives in per-partner history, not per-booking geometry.**

This is not a Genie problem. Partner integrity is owned by Wiom's Quality OS, Exit OS, and Enforcement OS. Genie consumes their state at the gate.

### 6a.2 — Why integrity is consumed, not computed
...
### 6a.3 — Three source OSes, two clocks
...
### 6a.4 — What crosses the membrane
...
### 6a.5 — Damping and failure modes to engineer

Both reviewers flagged amplification modes the bilateral contracts must dampen:
...
| Integrity shock (§6a) on the otherwise-eligible partner | — | **No payment** | Booking re-ranked past flagged partner; if no eligible partner remains, treated as Serviceability LOW. |
...
### 7.5 — Remediated-HIGH × MID is treated differently (v4 hidden cell)

A booking that entered at Gate 1 MID and was remediated (Street View + video + CRE) to HIGH, then arrives at Gate 2 MID, carries **two compounding remediation risks**. Geoff flagged this as the worst-retention cohort v4 treated identically to native-HIGH × MID.

**v5 rule.** Remediated-HIGH × MID bookings require **mandatory verify-visit** (not optional). Verify-visit cost is waived for these bookings — Wiom absorbs the cost because the upstream remediation was already paid for in customer friction. This prevents the double-remediation compounding while preserving the verify-visit loop's integrity.

---

## §8 — Principles of build

v3 and v4 shared nine principles. v5 preserves them verbatim (they are structural) and adds two new ones drawn from the v4 reviews.
...
**System-wide principles (both gates + shock channel)**
...
**P8. Cause-code fidelity.** Downstream failures tagged by type — GPS_TRUST / ADDRESS_RESOLUTION / SPATIAL / OPERATIONAL / INTEGRITY. Never lumped. Without this, no loop learns.
...
**P11. Quality vs Confidence separation (new in v5).** Every scored stock stores a `(quality, confidence, last_observed)` triplet, not a scalar. A HIGH with thin evidence is not the same asset as a HIGH with thick evidence.

**P12. Decay on all stocks (new in v5).** Every accumulating stock has a declared decay mechanism (time-weighted, evidence-weighted, or explicit recovery on clean evidence). A stock without decay is a leak.

---

## §9 — The system flow

Six stages plus a control pane. The control pane now includes the integrity-shock intake.
...
```
            ...
            │     ← integrity state (Phase 4+) consulted   │
            │        for veto on eligible partner          │
            └──────────────────────┬───────────────────────┘
                                   │  on bind
                                   ▼
            ┌──────────────────────────────────────────────┐
    ...
    ║  ← integrity events from Q-OS / XS-OS / Enforcement OS (Phase 4+)      ║
    ╚════════════════════════════════════════════════════════════════════════╝
```

Two structural fixes vs v4:
- **Stage 4 now names its drain** (install / cancel / 48h-SLA-miss) — v4 left the active-promise stock without a flow-out (Donna's leak finding).
- **Stage 6 renamed "immutable history (H)" and labelled "log not state"** — to prevent confusion with state stocks.

---

## §10 — Feedback loops (decay declared on every loop)

v4 had six loops; four lacked explicit decay. v5 declares decay on every loop and adds one new loop for the integrity channel.

Loop shape (same for all seven):
...
**Visible to source:** **new in v5** — partner sees his own team's movement-derived polygon expansion in his app, with attribution ("*your team passed through 3 new hexes this week; those hexes are now serviceable for you*"). Closes the silent-membrane gap Donna flagged.
...
### Loop 7 — Integrity shocks → partner corrections (new in v5)
...
**Visible to source:** partner sees quality state on his own dashboard via Data Access OS, in Q-OS's language, before any consequence fires (P15 visibility delay). This is the load-bearing property: **the partner learns why before the gate closes.**

### The seven loops at a glance
...
| 5 | technician GPS | visitation density per (partner, hex, week) | rolling 8-week window | **yes** (new in v5) |
| 6 | system outcome messaging | N/A (information flow) | N/A | yes (by construction) |
| 7 | external OS shocks | lives in Q-OS/XS-OS/Enforcement | owned by originating OS | yes (partner dashboard, P15 delay) |

---

## §11 — User involvement, named explicitly

Three shifts from today, preserved from v4.
...
New in v5: **the partner is also treated as an active participant, not a scored object.** The integrity channel (§6a) routes partner feedback through the OS the partner already interacts with (Q-OS → his own dashboard), not via Genie's opaque gate decisions. The partner knows why his polygon shrank, why his trust state changed, and what window he has to self-correct.

---

## §12 — Capabilities: what to build, what to reuse

Same four groups as v4, with deltas marked **(v5)** for changes.
...
| A6 | Two-round corrective loop + CRE callback | N | Medium | Q3 |
| A7 | Fallback text capture | N | Medium | Q3 |
| A8 | Per-mobile jitter-handling path **with recovery (v5)** | N | Medium | Q4 |
| A9 | Google Street View pull | N | Medium | Q4 |
| A10 | NER parsing for A7 | N | Medium | Q4 |
| A11 | User Address Confidence Model v0 scorer **storing triplet state (v5)** | N | Quick | Q1 |
| A12 | Customer transparency UI | N | Quick | Q1 |
| A13 | Repeat-customer friction reduction | N | Medium | Q4 |
| A14 | **SR-OS queue for MID remediation (v5)** — named SLA, volume ceiling, CP shock on breach | N | Quick | Q1 |

### B. Gate 2 capabilities (Serviceability Confidence)

| # | Capability | N/R | Impact | Q |
|---|---|---|---|---|
| B1 | BM1 activation at the gate **— polygon-only containment (v5)** | R | Medium | Q4 |
| B2 | Promise / ask-partner / verify-visit / reject governance | N | Quick | Q1 |
| B3 | Active promise exposure stock **with explicit drain on install/cancel/48h-SLA-miss (v5)** | N | Medium | Q3 |
| B4 | Technician / team GPS ingestion | N | Medium | Q4 |
| B5 | Landmark-grounded serviceability | N | Medium | Q3 |
| B6 | BM2 activation | R | Medium | Q4 |
| B7 | Exploration quota **with partner rotation (v5)** | N | Medium | Q4 |
| B8 | Paid verify-visit flow **with bonus < HIGH steady-state (v5 damping)** | N | Medium | Q3 |
...
| C9 | **Technician team-trail visibility to partner (v5)** — closes Loop 5's source-awareness gap | N | Quick | Q2 |
...
| D4 | Landmark-confidence accumulation **with triplet state + decay (v5)** | N | Medium | Q4 |
...
| D8 | Cause-code taxonomy extension (**+ INTEGRITY class v5**) | N | Quick | Q1 |
| D9 | Customer outcome transparency loop | N | Quick | Q1 |

### E. Cross-OS integrity channel (Phase 4 — all new in v5)
...
**Counts.** A = 14, B = 8, C = 9, D = 9, E = 8. Total **48** capabilities. **R (reuse) = 3** (B1, B6, C2). **N (new) = 45**.

The headline shift from v4: **8 new capabilities in group E** — the cross-OS integrity channel. This is the v5-specific work product and closes the 30.8pp gap Geoff identified.
...
**Q1 — Now, Quick impact.** B2 governance gate (Phase 1, polygon-only) · C1 structured notification · C2 serviceable-area map · C4 decline consequence · C5 ask-partner flow · A11 User Address Confidence v0 scorer · A12 customer transparency UI · A14 SR-OS queue · D8 cause-code taxonomy (+INTEGRITY) · D9 customer outcome transparency · E1 fast-path integrity contract · E8 bilateral contract terms.

**Q2 — Next month, Quick impact.** C3 decline-zones · C6 Street View partner-side · C7 verify-visit reward · C8 on-ground assist · C9 team-trail visibility to partner · E6 partner integrity dashboard.

**Q3 — Now, Medium-term.** A1-A7 capture substrate · B3 active promise stock (with drain) · B5 landmark-grounded serviceability · B8 verify-visit flow · D5 immutable history · E2 slow-path quality contract · E3 terminal-integrity contract · E4 booking-coord anomaly detector.

**Q4 — Next month, Medium-term.** A8 jitter-handling (with recovery) · A9 Street View customer-side · A10 NER · A13 repeat-customer friction · B1 serviceability model activation (Phase 2) · B4 technician GPS · B6 allocation ranking model activation (Phase 3) · B7 exploration quota (with rotation) · D1-D4 + D6-D7 feedback plumbing · E5 epistemic-uncertainty treatment · E7 post-clear probation.

**Reading the matrix.** **Phase 1 is Q1 + Q3-capture** — polygon-only Gate 2 + capture substrate. Ships business value without waiting for model activation. Later phases layer on: Phase 2 (serviceability model), Phase 3 (allocation ranking), Phase 4 (integrity channel). Within Phase 4, the fast-path integrity contract (E1) is highest-priority — without it, first-offense gaming remains undetected at promise time.

---

## §14 — How we'll know it worked

Ten signals grouped by claim + one gameability control.
...
- **MID → HIGH hex graduation rate over 90 days** (new — preserves Donna's decay check): target baseline TBD Sprint 1.

**Downstream friction drops:**
- Calls per pair: **1.92 → <1.3**.
- Gali-stuck call rate: **7.4% → <2%**.

**Integrity channel is active (v5):**
...
**Gameability control (v5 — addresses Geoff's §14 critique):**
...
Lag: most metrics 2-4 weeks. Leading operational signals — verification-completion, ask-partner acceptance, shock-query rate — observable in hours.

---

## §15 — Open questions

Carried forward + new from v5:
...
6. **Fast-path FPV detector design (E4)** — ownership, feature set, false-positive rate tolerance.
7. Customer transparency copy.
8. MID serviceability ops throughput ceiling.
9. **Bilateral shock-commit contract details (E1, E2, E3, E8)** — TTL, resolution SLA, cooldown ownership, graduated severity parameters.
10. **Epistemic-uncertainty floor shape (E5)** — how B down-weights sparse-evidence partners.
11. Remediated-HIGH × MID verify-visit waiver — ops cost absorption mechanism.

---

## §16 — Why fixing only one side leaves installs broken

- **Only Gate 1 →** cleaner GPS; partner still sees unstructured blob; serviceability false-negatives continue because R&D models aren't activated; gaming partners continue at 30.8pp gap.
- **Only Gate 2 →** BM1/BM2 activated; but address is still noisy; gaming partners still pass.
- **Only §6a integrity channel →** gaming partners filtered out; but the 25.7% structural drift and the 1.92 calls/pair remain.
- **All three →** three parties hold the same location model at the same quality at every handoff; gaming is surfaced as a constitutional property; voice call becomes confirmation, not discovery.

The three workstreams share the capture substrate and the S stock. Fixing them in parallel is cheaper than fixing them serially.

---

## §17 — What changes from v4 to v5

For readers who know v4:

1. **Fourth evidentiary claim added** — partner-side booking-coord gaming (Master Story Appendix). Justifies §6a.
2. **§6a Partner Integrity Channel added** — cross-OS routing via an already-existing integrity-event contract. Three source OSes (Enforcement fast-path, Q-OS slow-path, XS-OS terminal). Damping mechanics explicit.
3. **Phased rollout introduced (§2.7)** — Phase 1 ships on polygon-only containment without waiting for model activation. Phase 2 adds the serviceability model; Phase 3 adds allocation ranking; Phase 4 adds the integrity channel. Capabilities in §12 inherit phase tags implicitly from their quadrant.
4. **§6.3 reworked** — containment (polygon-only) is structurally separated from tier decision (phase-dependent enrichments). Containment never depends on models.
5. **MID decline now shrinks polygon** (softer than HIGH) with consequence shown pre-confirm. Previously stated as "polygon unchanged" — corrected.
6. **§7 Payment rewritten** — take at promise, auto-refund if MID unbound in 48h.
7. **Remediated-HIGH × MID cell restored** — mandatory verify-visit with cost waived.
8. **§10 Feedback loops rebuilt** — decay declared on every loop, triplet state on every scored stock, seventh loop added for integrity.
9. **§9 Stage 4 drain named** — active promise stock is no longer a leak.
10. **§11 extended** — partner treated as active participant via his own quality dashboard.
11. **§12 gains group E** — 8 new cross-OS capabilities.
12. **§14 metrics** — held-volume conversion and expansion-queue graduation added as gameability controls; MID→HIGH graduation and recidivism as oscillation controls.
13. **Principles P11 (quality vs confidence triplet) and P12 (decay mandatory) added** as first-class frame principles.
14. **Genie internals moved to Appendix A** — body uses plain language (integrity events, bilateral contract, etc.), technical stock names (B/R/E/S/H/CP) and contract identifiers (IC-INT-10) live in the appendix for system-spec readers.
15. **§17 delta list** — so future readers can trace what each review cycle produced.
...
- **S (Shock Ledger)** — events from outside Genie that demand re-evaluation. The Partner Integrity channel (§6a) enters Genie here.
...
The Partner Integrity Channel (§6a) enters Genie through **IC-INT-10 / IC-INT-10a** — the Shock Ledger Intake Contract, already specified. The missing engineering work is three bilateral shock-commit contracts:
...
At the gate, G queries S at the existing `integrity_block` and `execution_path_available` gate points (V5 Spec §5.5.1).

---

## Appendix B — Seeds for the system architect document

- Cause-code taxonomy extension including the new INTEGRITY class.
...
- Gaming-score vs trust-score separation — trust drives re-capture; gaming drives human review.
...
- Booking-coord anomaly detector specification — splitter-share pattern, coord-cluster anomaly, detection threshold, FPV classification rules.
- Epistemic-uncertainty treatment on B's partner-side features — formal treatment of sparse-evidence down-weighting.
- Post-clear probation state machine.
- Remediated-HIGH × MID verify-visit waiver accounting — cost absorption pathway.
- Phase gating criteria — what operational evidence must land before Phase 2, Phase 3, Phase 4 activate.
